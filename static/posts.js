...
function view_comment(id){
    ...
    c_blocks.push(id);
    new_block = '<div id="comment_block_' + id + '"><button onclick="close_comment()" class="close_comment_button">Cancel</button></div>';
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            var upload_comment = '<div id="new_comment_form_' + post.id + '"><center><h1>New Comment</h1><center><b id="message"></b></center><textarea id="comment_article_' + post.id + '" maxlength="1000" style="width:500px; padding: 10px; height: 200px; border-radius: 10px;"></textarea><input type="file" id="comment_media_' + post.id + '" name="media" onchange="validate_c_media(this)" accept="image/*,video/mp4" value="" hidden><div id="label_cont"><label for="comment_media_' + post.id + '" id="media_label_' + post.id + '">Upload Media &#128206;</label></div><button onclick=submit_comment("' + post.id + '") id="button" data-mdb-ripple-color="dark" class="submit_comment_button">Comment</button></center></div>'
